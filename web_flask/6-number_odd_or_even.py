...
# Defne the route '/python/(<text>)'
@app.route('/python/', defaults={'text', 'is cool', strict_slashed=False)
@app.route('/python/<text>', strict_slashes=False)
def python_with_text(text):
        """Display 'python' followed by the value of <text>.
        Replaces any underscores in <text> with with slashes.
	"""
	# Replace underscores with spaces in the text variable
	formatted_text = text.replace('_', ' ')
	return "python{}".format(formatted_text)

# Defne the route '/number_odd_or_even/<n>'
@app.route('/number_odd_or_even/<int:n>', strict_slashed=False)
def number_odd_or_even(n):
        """Displays an HTML page only if <n> is an integer.
	States whether <n> is odd or even is the body.
	"""
	...
	if isinstance(n, int):
		...
		# value of n and even or odd to the template
...
# Defiine the route for '/number_template/<n>'
